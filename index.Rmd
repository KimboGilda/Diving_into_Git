...
- type `git init`.  
The later creates a new subdirectory named __.git__ that contains all of your necessary repository files. At this point, in case there are existing files in your directory, they have to be tracked. To do so, we do the following:

- type `git add .` > so we can add all the non-tracked files to the staging area _Fig_.\@ref(fig:skeleton).
...
In the second, quite common use case scenario, we are dealing with the cloning of an existing repository. To do so, we just have to copy the respective url, using the graphical user interface (gui) of a respective git hosting platform. In the figure below fig.\@ref(fig:url), we are showcasing how this is performed with __github__.
